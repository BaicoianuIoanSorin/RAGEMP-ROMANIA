...
function playerQuitHandler(player, exitType, reason)
{
    ...
    if(player.loggedInAs == true)
    {
        ...
        //Destroy player rent bikes
        if(player.data.rentedBike)
        {
            player.data.rentedBike.destroy();
            player.data.rentedBike = null;
        }
...
mp.events.add("loadVariables", player => {
    ...
    player.vehicleValid = (name) => {
        ...
        return (vehiclesData.includes(name));
    }

    player.vehicleModelHaveEngine = (name) => {

        return (vehiclesWithoutEngines.includes(name));
    }
    ...
    player.isAtJob = (x) => {
        ...
        return (!!player.IsInRange(struct.jobs[x].jobWorkPosX, struct.jobs[x].jobWorkPosY, struct.jobs[x].jobWorkPosZ, 5));
    }
